...
4. **Architecture** – `src/ssi/investigator/orchestrator.py` is the investigation entry point. `src/ssi/osint/` contains passive recon modules. `src/ssi/browser/` handles Playwright automation. `src/ssi/identity/vault.py` generates synthetic PII. `src/ssi/store/scan_store.py` persists scan results and creates case records via direct DB writes. **Read `core/.github/architecture-cheatsheet.instructions.md` at session start** — it covers UI↔API proxy routing, auth model per environment, SSI↔Core integration, storage flows, and common pitfalls that have wasted cycles in the past.
...
9. **External Integrations** – SSI writes investigation results directly to the shared database via `ScanStore.create_case_record()`. This creates case records with wallet indicators and OSINT entities. Keep schema writes in sync with core's table definitions.
